...
const startWorker = (workerId) => {
  ...
  app.get('/api/v1/streaming/union', (req, res) => {
    const onlyMedia = req.query.only_media === '1' || req.query.only_media === 'true';
    const channel   = onlyMedia ? 'timeline:union:media' : 'timeline:union';
  ...
  wss.on('connection', ws => {
    ...
    case 'union':
      ...
      break;
    case 'union:media':
      streamFrom('timeline:union:media', req, streamToWs(req, ws), streamWsEnd(req, ws), true);
      break;
    case 'direct':
